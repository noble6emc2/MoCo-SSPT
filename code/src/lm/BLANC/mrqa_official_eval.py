...
def f1_en_score(prediction, ground_truth):
    prediction_tokens = normalize_en_answer(prediction).split()
    ground_truth_tokens = normalize_en_answer(ground_truth).split()
    common = Counter(prediction_tokens) & Counter(ground_truth_tokens)
    num_same = sum(common.values())
    if num_same == 0:
        return [0, 0, 0]
